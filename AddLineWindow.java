...
public class AddLineWindow {
    ...
    public AddLineWindow(ArrayList<OrderedPane> layers, VBox layerContainer, StackPane viewer, Gui gui) {
    ...
    public AddLineWindow(ArrayList<OrderedPane> layers, VBox layerContainer, StackPane viewer, OrderedPane originalLayer, Gui gui) {
        // create pane and grid pane
        newLineWindow = new Stage();
        pane = new GridPane();
        first = false;
    ...
    private Boolean isValidLine(String startX, String startY, String endX, String endY, String startChangeInX,
        ...
        if (startX != null && isNumeric(startX)
                ...
                // check if a valid expression (also checks if numeric)
                && startChangeInX != null && isExpression(startChangeInX)
                && startChangeInY != null && isExpression(startChangeInY)
                && endChangeInX != null && isExpression(endChangeInX)
                && endChangeInY != null && isExpression(endChangeInY)
    ...
    // tests if the input is numeric or can be converted and evaluated as an expression
    private boolean isExpression(String str) {
        if (isNumeric(str)) {
            return true;
        }
        try {
            String postfixExpression = InfixToPostfix.convert(str, 1);
            PostfixExpression.evaluate(postfixExpression);
            return true;
        } catch (Exception e) {
            return false;
        }
